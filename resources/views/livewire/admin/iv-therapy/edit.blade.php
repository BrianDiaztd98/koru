...
<head>
    ...
    <title>Edit IV Therapy — Koru CMS</title>
...
<body class="min-h-full bg-slate-950 text-slate-100 antialiased selection:bg-[#0EB3B9]/30 selection:text-white relative overflow-x-hidden">
    ...
    <div class="relative z-10 min-h-screen flex flex-col">
        <livewire:admin.admin-topbar :title="$title ?? 'Edit IV Therapy'" />

        <main class="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-10 flex-1">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">

                <livewire:admin.admin-sidebar :dashboard="false" :activeTarget="'iv_therapy'" />
            ...
            <form action="{{ route('admin.services.update', $service) }}" method="POST">
                @csrf
                @method('PUT')
                <input type="hidden" name="category" value="iv_therapy">
                ...
                <div class="grid grid-cols-1 gap-8 lg:grid-cols-3 items-start">
                    ...
                    <div class="lg:col-span-2 rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-xl p-6 sm:p-8 shadow-xl shadow-black/30 space-y-6">
                        ...
                        <div class="flex items-center gap-4 pb-5 border-b border-slate-800/60">
                            ...
                            <div>
                                <h2 class="text-xl font-bold text-white tracking-tight">IV Therapy</h2>
                        ...
                        <div class="prose prose-invert max-w-none space-y-4">
                            @include('livewire.admin.iv-therapy._form')
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-xl p-6 shadow-xl shadow-black/30 space-y-4">
                            ...
                            <div class="pt-2 flex flex-col gap-3">
                                ...
                                <a href="{{ route('admin.services.index', ['category' => 'iv_therapy']) }}" 
                                   class="w-full inline-flex items-center justify-center rounded-xl border border-slate-800 bg-slate-950/40 px-5 py-3 text-xs font-semibold text-slate-400 backdrop-blur-sm transition-all duration-150 hover:bg-slate-900/60 hover:border-slate-700 hover:text-slate-200">
                                    Cancel Changes
                                </a>
                            </div>
                        </div>
