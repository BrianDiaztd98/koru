<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    ...
    <div>
        ...
        @if(!empty($service->image_path))
            @if(($service->category ?? '') === 'recovery_performance')
                <div class="mt-3 flex items-center gap-4">
                    <div class="w-28 h-20 rounded-lg overflow-hidden border border-slate-800 bg-slate-900 flex items-center justify-center">
                        <img src="{{ str_starts_with($service->image_path, 'img/') ? asset($service->image_path) : asset('storage/' . $service->image_path) }}" alt="Service image" class="object-cover w-full h-full" />
                    </div>
                    <div class="text-xs text-[#0EB3B9] font-mono">
                        <div>Current image</div>
                        <div class="text-slate-300 underline font-sans">{{ basename($service->image_path) }}</div>
                    </div>
                </div>
            @else
