...
<div class="lg:col-span-3 space-y-8 animate-fadeIn text-slate-200">
    ...
    <div class="rounded-3xl border border-slate-800/80 bg-slate-900/20 backdrop-blur-xl p-6 sm:p-8 shadow-2xl shadow-black/40 relative overflow-hidden">
        ...
        <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between relative z-10">
            <div>
                <p class="font-mono text-xs font-bold uppercase tracking-[0.24em] text-[#0EB3B9]">Management Dashboard</p>
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 shrink-0">
                <a href="{{ route('admin.about.index') }}" class="group flex items-center justify-center rounded-xl border border-slate-800 bg-slate-950/60 px-5 py-3.5 text-xs font-mono font-bold uppercase tracking-wider text-slate-300 transition-all duration-200 hover:border-[#0EB3B9]/40 hover:bg-[#0EB3B9]/5 hover:text-[#0EB3B9] active:scale-[0.98]">
                    <svg class="w-4 h-4 mr-2 text-slate-500 group-hover:text-[#0EB3B9] transition-colors" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <a href="{{ route('admin.services.create') }}" class="group flex items-center justify-center rounded-xl border border-slate-800/80 bg-[#0EB3B9] px-5 py-3.5 text-xs font-mono font-bold uppercase tracking-wider text-white shadow-lg shadow-[#0EB3B9]/10 transition-all duration-200 hover:bg-[#0E788D] hover:shadow-[#0E788D]/20 active:scale-[0.98]">
        ...
        <div class="rounded-3xl border border-slate-800/80 bg-slate-900/20 backdrop-blur-xl p-6 shadow-xl shadow-black/20 flex flex-col justify-between space-y-4">
            <div>
                ...
                <div class="mt-3 flex items-baseline gap-2">
                    ...
                    <span class="font-mono text-xs text-[#0EB3B9] uppercase font-bold tracking-widest">Active Units</span>
                </div>
            </div>
            <p class="text-xs leading-relaxed text-slate-400">
        ...
        <div class="rounded-3xl border border-slate-800/80 bg-slate-900/20 backdrop-blur-xl p-6 shadow-xl shadow-black/20 flex flex-col justify-between space-y-4">
            <div>
                ...
                <div class="mt-3 flex items-baseline gap-2">
                    ...
                    <span class="font-mono text-xs text-[#0EB3B9] uppercase font-bold tracking-widest">Active Plans</span>
                </div>
            </div>
            <p class="text-xs leading-relaxed text-slate-400">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <div class="mt-3 flex items-baseline gap-2">
                        ...
                        <span class="font-mono text-xs text-[#0EB3B9] uppercase font-bold tracking-widest">Visits in {{ $selectedYear }}</span>
                    </div>
                </div>
                ...
                <form method="GET" action="{{ route('admin.management.index') }}" class="flex items-center gap-2">
                    ...
                    <select id="year" name="year"
                        class="rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2 text-sm text-slate-200 focus:border-[#0EB3B9] focus:outline-none">
                    ...
                    <button type="submit" class="rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2 text-sm font-semibold text-slate-300 transition hover:border-[#0EB3B9]/40 hover:text-[#0EB3B9]">
                        Apply
                    </button>
                </form>
            </div>
                ...
                <div class="space-y-2.5 max-h-[310px] overflow-y-auto pr-2 koru-scrollbar">
                    @foreach ($categoryCounts as $categoryKey => $count)
                        <div class="group flex items-center justify-between rounded-xl bg-slate-950/40 border border-slate-800/50 px-4 py-3 text-sm text-slate-300 transition-colors hover:border-slate-800 hover:bg-slate-950/80 relative overflow-hidden">
                            <div class="absolute left-0 top-0 bottom-0 w-[2px] bg-slate-800 group-hover:bg-[#0EB3B9] transition-colors"></div>
