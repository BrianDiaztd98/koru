<div class="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/80 p-4" wire:click.self="closeDeleteModal">
    <div class="w-full max-w-md overflow-hidden rounded-2xl border border-slate-800/70 bg-slate-900 shadow-2xl">
        <div class="border-b border-slate-800/70 px-6 py-5">
            ...
            <p class="text-sm text-slate-400">This action will permanently delete slide <span class="font-mono text-white font-bold">[{{ sprintf('%02d', $slideToDelete->id) }}] {{ $slideToDelete->title_line_1 }}</span>. This operation cannot be undone. All associated media assets will be purged from storage.</p>
        </div>
        ...
        <div class="p-6">
            <div class="rounded-xl border border-slate-800/60 bg-slate-950/30 p-4 mb-6">
                <div class="flex items-center gap-2 text-xs font-mono text-slate-500 mb-2">
                    <span>TARGET_ID:</span>
                    <span class="text-rose-400 font-bold">[{{ sprintf('%02d', $slideToDelete->id) }}]</span>
                </div>
                <div class="flex items-center gap-2 text-xs font-mono text-slate-500">
                    <span>STATUS:</span>
                    <span class="{{ $slideToDelete->is_active ? 'text-emerald-400' : 'text-slate-500' }} font-bold">
                        {{ $slideToDelete->is_active ? 'ACTIVE_NODE' : 'INACTIVE_NODE' }}
                    </span>
                </div>
            </div>
