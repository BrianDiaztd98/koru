            ...
            <div class="lg:col-span-2 rounded-2xl border border-slate-800/80 bg-slate-900/20 backdrop-blur-xl p-6 sm:p-8 shadow-2xl shadow-black/40 space-y-6 relative overflow-hidden">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800/60">
                    <div class="flex items-center gap-4">
                        <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#02B8BC]/10 text-[#02B8BC] border border-[#02B8BC]/20 shadow-inner">
                ...
                <div x-show="activeTab === 'copy'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2" class="space-y-5">
                    <div class="grid gap-5 lg:grid-cols-2">
                        <div>
                            ...
                            <input wire:model="title" id="title" type="text" value="{{ $title }}" maxlength="100" class="w-full rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-3 text-sm text-slate-200 outline-none transition-all duration-200 shadow-inner focus:border-[#02B8BC] focus:ring-2 focus:ring-[#02B8BC]/10 placeholder:text-slate-600" placeholder="Main header title (max 100 chars)" />
                        ...
                        <div>
                            ...
                            <input wire:model="subtitle" id="subtitle" type="text" value="{{ $subtitle }}" maxlength="150" class="w-full rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-3 text-sm text-slate-200 outline-none transition-all duration-200 shadow-inner focus:border-[#02B8BC] focus:ring-2 focus:ring-[#02B8BC]/10 placeholder:text-slate-600" placeholder="Optional sub-heading (max 150 chars)" />
                    ...
                    <div>
                        ...
                        <textarea wire:model="description" id="description" rows="4" x-ref="description" @input="updateCount('description')" class="w-full rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-3 text-sm text-slate-200 outline-none transition-all duration-200 shadow-inner focus:border-[#02B8BC] focus:ring-2 focus:ring-[#02B8BC]/10 placeholder:text-slate-600" placeholder="Primary narrative content... (max 2000 chars)">{{ $description }}</textarea>
                    ...
                    <div class="grid gap-5 lg:grid-cols-2 pt-2">
                        <div class="p-4 rounded-xl border border-slate-800/60 bg-slate-950/20">
                            <label for="philosophy" class="block font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 text-[#02B8BC] flex items-center gap-1">
                                Core Philosophy <span class="text-rose-400">(*)</span>
                            </label>
                            <textarea wire:model="philosophy" id="philosophy" rows="4" x-ref="philosophy" @input="updateCount('philosophy')" class="koru-scrollbar max-h-40 w-full overflow-y-auto rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-sm text-slate-200 outline-none transition-all duration-200 focus:border-[#02B8BC] focus:ring-1 focus:ring-[#02B8BC]/20 placeholder:text-slate-600" placeholder="Philosophy details... (max 2000 chars)">{{ $philosophy }}</textarea>
                            <div class="mt-1.5 flex justify-between text-xs">
                        ...
                        <div class="p-4 rounded-xl border border-slate-800/60 bg-slate-950/20">
                            <label for="vision" class="block font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 text-[#02B8BC] flex items-center gap-1">
                                Misión / Visión <span class="text-rose-400">(*)</span>
                            </label>
                            <textarea wire:model="vision" id="vision" rows="4" x-ref="vision" @input="updateCount('vision')" class="koru-scrollbar max-h-40 w-full overflow-y-auto rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-sm text-slate-200 outline-none transition-all duration-200 focus:border-[#02B8BC] focus:ring-1 focus:ring-[#02B8BC]/20 placeholder:text-slate-600" placeholder="Misión y visión... (max 2000 chars)">{{ $vision }}</textarea>
                            <div class="mt-1.5 flex justify-between text-xs">
                                <span class="text-slate-500">Required</span>
                    ...
                    <div class="p-4 rounded-xl border border-slate-800/60 bg-slate-950/20">
                        <label for="mission" class="block font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 text-[#02B8BC] flex items-center gap-1">
                            Mission Statement <span class="text-slate-500">(Opcional)</span>
                        </label>
                        <textarea wire:model="mission" id="mission" rows="4" x-ref="mission" @input="updateCount('mission')" class="koru-scrollbar max-h-40 w-full overflow-y-auto rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-sm text-slate-200 outline-none transition-all duration-200 focus:border-[#02B8BC] focus:ring-1 focus:ring-[#02B8BC]/20 placeholder:text-slate-600" placeholder="Mission statement... (max 2000 chars)">{{ $mission }}</textarea>
                        <div class="mt-1.5 flex justify-between text-xs">
                ...
                <div x-show="activeTab === 'glance'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2" class="space-y-6">
                    <div class="p-5 rounded-xl border border-slate-800/80 bg-slate-950/30 space-y-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2 text-xs font-mono font-bold text-[#02B8BC] uppercase tracking-widest">
                                <span class="h-1.5 w-1.5 rounded-full bg-[#02B8BC]"></span> KORU at a Glance
                            </div>
                            ...
                            @foreach($glanceItems as $index => $item)
                                <div class="p-4 rounded-xl border border-slate-800/60 bg-slate-950/20 space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <input wire:model="glanceItems.{{ $index }}.title" type="text" maxlength="80" class="w-full rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-[#02B8BC]" placeholder="Item title (max 80 chars)" />
                                    ...
                                    <div>
                                        ...
                                        <textarea wire:model="glanceItems.{{ $index }}.description" rows="2" class="w-full rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-[#02B8BC]" placeholder="Short explanation... (max 500 chars)"></textarea>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                        ...
                        <div class="p-4 rounded-xl border border-slate-800 bg-slate-950/40 flex flex-col justify-between items-center text-center space-y-3">
                            <div class="w-full aspect-[4/3] rounded-lg bg-slate-900 border border-slate-800 flex items-center justify-center overflow-hidden relative group">
                                ...
                                @else
                                    <span class="text-xs font-mono text-slate-600 group-hover:text-[#02B8BC] transition-colors">IMG_01_EMPTY</span>
                                @endif
                            </div>
                            <div class="w-full">
                                ...
                                <input wire:model="image_1" type="file" accept="image/*" class="w-full text-[11px] text-slate-500 file:mr-2 file:rounded-md file:border-0 file:bg-[#02B8BC]/10 file:px-2.5 file:py-1 file:font-mono file:text-[10px] file:font-bold file:text-[#02B8BC] cursor-pointer" />
                        ...
                        <div class="p-4 rounded-xl border border-slate-800 bg-slate-950/40 flex flex-col justify-between items-center text-center space-y-3">
                            <div class="w-full aspect-[4/3] rounded-lg bg-slate-900 border border-slate-800 flex items-center justify-center overflow-hidden relative group">
                                ...
                                @else
                                    <span class="text-xs font-mono text-slate-600 group-hover:text-[#02B8BC] transition-colors">IMG_02_EMPTY</span>
                                @endif
                            </div>
                            <div class="w-full">
                                <label class="block font-mono text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                                ...
                                <input wire:model="image_2" type="file" accept="image/*" class="w-full text-[11px] text-slate-500 file:mr-2 file:rounded-md file:border-0 file:bg-[#02B8BC]/10 file:px-2.5 file:py-1 file:font-mono file:text-[10px] file:font-bold file:text-[#02B8BC] cursor-pointer" />
                                <p class="mt-1 text-[10px] text-slate-600">JPG, PNG, WebP • Max 2MB</p>
                        ...
                        <div class="p-4 rounded-xl border border-slate-800 bg-slate-950/40 flex flex-col justify-between items-center text-center space-y-3">
                            <div class="w-full aspect-[4/3] rounded-lg bg-slate-900 border border-slate-800 flex items-center justify-center overflow-hidden relative group">
                                ...
                                @else
                                    <span class="text-xs font-mono text-slate-600 group-hover:text-[#02B8BC] transition-colors">IMG_03_EMPTY</span>
                                @endif
                            </div>
                            <div class="w-full">
                                <label class="block font-mono text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                                ...
                                <input wire:model="image_3" type="file" accept="image/*" class="w-full text-[11px] text-slate-500 file:mr-2 file:rounded-md file:border-0 file:bg-[#02B8BC]/10 file:px-2.5 file:py-1 file:font-mono file:text-[10px] file:font-bold file:text-[#02B8BC] cursor-pointer" />
                                <p class="mt-1 text-[10px] text-slate-600">JPG, PNG, WebP • Max 2MB</p>
                        ...
                        <div class="p-4 rounded-xl border border-slate-800 bg-slate-950/40 flex flex-col justify-between items-center text-center space-y-3">
                            <div class="w-full aspect-[4/3] rounded-lg bg-slate-900 border border-slate-800 flex items-center justify-center overflow-hidden relative group">
                                ...
                                @else
                                    <span class="text-xs font-mono text-slate-600 group-hover:text-[#02B8BC] transition-colors">IMG_04_EMPTY</span>
                                @endif
                            </div>
                            <div class="w-full">
                                <label class="block font-mono text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                                ...
                                <input wire:model="image_4" type="file" accept="image/*" class="w-full text-[11px] text-slate-500 file:mr-2 file:rounded-md file:border-0 file:bg-[#02B8BC]/10 file:px-2.5 file:py-1 file:font-mono file:text-[10px] file:font-bold file:text-[#02B8BC] cursor-pointer" />
                                <p class="mt-1 text-[10px] text-slate-600">JPG, PNG, WebP • Max 2MB</p>
            ...
            <div class="space-y-4 lg:sticky lg:top-6">
                <div class="rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-xl p-6 shadow-xl shadow-black/20 space-y-4">
                    <div class="flex items-center justify-between pb-2 border-b border-slate-800/50">
                        <h3 class="text-xs font-bold uppercase tracking-widest text-[#02B8BC] font-mono">Update State</h3>
                        <div wire:loading.delay class="text-[10px] font-mono text-[#02B8BC] animate-pulse uppercase">Syncing...</div>
                    </div>
                    ...
                    <div class="pt-2 flex flex-col gap-3">
                        <button type="submit" 
                                class="w-full inline-flex items-center justify-center rounded-xl bg-[#02B8BC] px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-[#02B8BC]/10 transition-all duration-200 hover:bg-[#037E93] hover:shadow-[#037E93]/20 active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-[#02B8BC] focus:ring-offset-2 focus:ring-offset-slate-900 cursor-pointer">
