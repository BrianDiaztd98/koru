<header x-data="{ mobileMenuOpen: false }"
        class="relative z-50 border-b border-slate-800/80 bg-slate-950/90 backdrop-blur-md shadow-lg shadow-slate-950/20"
    ...
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <a href="#about-us" class="flex items-center gap-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0EB3B9] rounded-xl transition-transform active:scale-95" aria-label="KORU Center - Back to top">
            ...
            <img src="{{ asset('img/logo.webp') }}" alt="KORU Center" width="144" height="56" class="h-14 w-auto object-contain brightness-110" loading="eager" decoding="async" fetchpriority="high" />
        </a>
        ...
        <button @click="mobileMenuOpen = !mobileMenuOpen" 
                type="button"
                class="inline-flex items-center justify-center rounded-xl border border-slate-800 bg-slate-950/40 p-2.5 text-slate-400 shadow-sm transition-all hover:border-[#0EB3B9]/40 hover:text-white lg:hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0EB3B9]"
                aria-expanded="false"
    ...
    <div x-show="mobileMenuOpen" 
        ...
        <div class="space-y-2 px-4 py-4 shadow-inner">
            @if(isset($headerNavItems) && count($headerNavItems) > 0)
                @foreach($headerNavItems as $item)
                    <a href="{{ $item['href'] }}" @click="mobileMenuOpen = false" class="block rounded-xl px-4 py-3 text-sm font-medium text-slate-300 transition hover:bg-slate-900 hover:text-[#0EB3B9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0EB3B9]">{{ $item['label'] }}</a>
                @endforeach
            @else
