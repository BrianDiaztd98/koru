<div class="rounded-3xl border border-slate-800/80 bg-slate-950/50 p-6 sm:p-8" wire:key="course-enrollment-form">
    <div class="max-w-2xl">
        <p class="text-xs font-bold uppercase tracking-[0.2em] text-[#02B8BC]">Website registration</p>
    ...
    @else
        <form wire:submit="submit" class="mt-6 space-y-5">
            <div>
                ...
                <select id="ce-course" wire:model="course_id" required aria-required="true" class="mt-2 w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-sm text-white focus:border-[#02B8BC] focus:outline-none focus:ring-2 focus:ring-[#02B8BC]/20">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                <div>
                    <label for="ce-full-name" class="block text-xs font-bold uppercase tracking-wider text-slate-400">Full name <span class="text-[#02B8BC]">(*)</span></label>
                    <input id="ce-full-name" type="text" wire:model="full_name" autocomplete="name" required aria-required="true" class="mt-2 w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-sm text-white placeholder:text-slate-600 focus:border-[#02B8BC] focus:outline-none focus:ring-2 focus:ring-[#02B8BC]/20" placeholder="Your full name">
            ...
            <div>
                ...
                <textarea id="ce-message" wire:model="message" rows="4" class="mt-2 w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-sm text-white placeholder:text-slate-600 focus:border-[#02B8BC] focus:outline-none focus:ring-2 focus:ring-[#02B8BC]/20" placeholder="Questions or notes for our team"></textarea>
                @error('message') <span class="mt-1 block text-xs text-rose-300">{{ $message }}</span> @enderror
            </div>

            <button type="submit" wire:loading.attr="disabled" class="inline-flex items-center justify-center rounded-xl bg-[#02B8BC] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#037E93] disabled:cursor-wait disabled:opacity-60">
