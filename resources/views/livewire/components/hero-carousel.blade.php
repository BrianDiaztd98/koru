<div>
    @if(!empty($slides))
        <section id="hero-carousel"
                ...
                <div class="carousel-container relative h-full">
                    ...
                    @foreach($slides as $slide)
                        <div class="carousel-slide absolute inset-0 transition-all duration-1000 ease-out {{ $loop->first ? 'opacity-100 z-20' : 'opacity-0 z-10' }}"
                             :class="{ 'opacity-100 z-20': currentSlide === {{ $loop->index }}, 'opacity-0 z-10': currentSlide !== {{ $loop->index }} }"
                             role="tabpanel"
                             id="hero-slide-panel-{{ $loop->index }}"
                             aria-labelledby="hero-slide-tab-{{ $loop->index }}"
                             aria-roledescription="slide"
                             :aria-hidden="currentSlide !== {{ $loop->index }}"
                             :inert="currentSlide !== {{ $loop->index }}">
            ...
            <div class="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2" role="tablist" aria-label="Choose slide">
    ...
    @else
        <section id="hero-carousel" class="relative bg-slate-950 text-white overflow-hidden">
            <div class="relative h-screen max-h-[85vh] min-h-[680px] sm:min-h-[750px]">
                <div class="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/70 to-transparent"></div>
                <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_rgba(14,120,141,0.15)_0%,_transparent_60%)]"></div>
