<div id="booster_shots" class="panel transition-all duration-300 transform hidden">
    <div class="rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-xl p-6 shadow-xl shadow-black/20 space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-5 border-b border-slate-800/60 gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-bold text-white tracking-tight">{{ $categories['booster_shots'] ?? 'Booster Shots' }}</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Manage Booster Shots services.</p>
                </div>
            </div>
            <div>
                <a href="{{ route('admin.services.create', ['category' => 'booster_shots']) }}" class="inline-flex items-center justify-center rounded-xl bg-[#0EB3B9] px-3 py-2 text-xs font-semibold text-white shadow-lg shadow-[#0EB3B9]/10 transition-all duration-200 hover:bg-[#0E788D] hover:shadow-[#0E788D]/20 active:scale-[0.98]">Add {{ $categories['booster_shots'] ?? 'Booster Shots' }}</a>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-slate-800/80 bg-slate-950/20 p-5">
                <div class="flex items-center justify-between gap-4 mb-5">
                    <div>
                        <h3 class="text-sm font-semibold uppercase tracking-[0.24em] text-slate-400">{{ $categories['booster_shots'] ?? 'Booster Shots' }}</h3>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-800/60 text-left text-sm">
                            ...
                            <tbody class="divide-y divide-slate-800/40">
                                @forelse($services['booster_shots'] ?? [] as $service)
